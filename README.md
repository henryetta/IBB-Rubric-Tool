...
| File | Who it's for | What they see |
|---|---|---|
| `index.html` (needs `?domain=<name>`) | Participants | Consent screen → questionnaire → thank-you. Contains no admin/PIN code at all. If opened without a domain parameter, shows a neutral "no study link provided" message. |
| `admin.html` (needs the access code) | Researcher | Dashboard — create domains, generate respondent links, view aggregated results, export CSV. |

Generate a respondent link from the admin dashboard ("Copy respondent link" on any domain card) — don't hand-type the URL. Bookmark `admin.html` for your own use; never share it or the bare `index.html` link with participants.

## Setup — deploying this yourself

1. Create a public GitHub repository.
2. Add `index.html`, `admin.html`, and `ibb_shared.js` to the repo root.
3. Repo **Settings → Pages** → source: deploy from branch → `main`, folder `/ (root)`.
4. Wait a minute or two for the first deploy; GitHub will show the live URL under Settings → Pages.

No build step, no dependencies to install beyond the three files themselves.
